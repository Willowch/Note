...
# 1.创建会话逻辑:
#     0>前端post请求建立新会话,并返回会话唯一标识
#     1>根据时间生成唯一会话标识
#     2>保存标识,消息列表到sessions目录
#     3>向前端返回json
# 2.与ai交互逻辑:
#       0>前端post请求(发送此次用户消息)LLM返回此次响应消息
#       1>加载历史会话数据:json和字典对应
#       2>构建此次LLM交互的消息数据message:提示词+历史消息+此次请求问题
#       3>调用AI大模型 DeepSeek
#       4>获取响应的数据
#       5>将响应的数据接入message
#       6>保存数据到sessions中
...
#     2.获取会话文件名称,倒序后返回
# 4.点击加载指定会话逻辑:
#     1>前端get(将点击的会话标识通过路径参数发送)请求该会话历史数据
#     2>根据会话标识找到文件,读出并返回
# 5.删除指定会话逻辑:
#     1>前端delete请求(将点击的会话标识通过路径参数发送)删除该会话
#     2>根据会话标识找到文件,删除
#     3>返回删除成功,无响应数据
# 问题1:为什么后端数据改变,前端立刻响应?
# 因为js(vue的响应式绑定),将后端数据与前端组件绑定,故后端数据改变,前端会立刻响应
...
# 返回给前端的相应数据的封装,自动转换为json
class ApiResponse(BaseModel):
    code: int
    message: str
    data: Any  # 任意类型的数据

#前端请求数据封装,自动将json格式数据对应到py对象中;
...
def chat(request: ChatRequest) -> ApiResponse:
    ...
    with open(session_path, "r", encoding="utf-8") as f:
        session_data = json.load(f) #josn格式直接读为字典

    # 2. 构建AI大模型交互的消息数据
    messages = [{"role": "system", "content": SYSTEM_PROMPT}]
    for message in session_data["messages"]:#session_data["messages"]:中为字典的列表
    ...
    # 5. 跟新消息列表中的消息
    messages.pop(0) #保存时不保存提示词
    messages.append({"role": "assistant", "content": ai_response})#加上此次的响应数据
    session_data["messages"] = messages #更新记录数据
    logging.info(f"更新后的会话信息: {session_data}")

    # 6. 保存会话信息到json文件中
    with open(session_path, "w", encoding="utf-8") as f:#将记录数据从字典保存回json文件
        json.dump(session_data, f, ensure_ascii=False, indent=2)
...
def get_sessions() -> ApiResponse:
    ...
    session_files = os.listdir("sessions")#返回文件名组成的列表
...
@app.get("/api/sessions/{session_id}") #路径参数通配
def get_session(session_id: str) -> ApiResponse:#参数名称与路径参数名称一致
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="127.0.0.1", port=8000) # access_log=False : 关闭访问日志参数